 ...
 */
 ...
 function CitySelect(options) {
	...
	this.config = jq.extend(this.config, options || {});
	this.init();
 }
 ...
 CitySelect.prototype = {
	...
	init: function() {
		...
		this.provCode = jq(_config.provId).attr("data-code");
		this.cityCode = jq(_config.cityId).attr("data-code");
		this.areaCode = jq(_config.areaId).attr("data-code");
		...
		var provFirstOpt = jq(_config.provId).val();
		if(provFirstOpt == -1) {
			jq(_config.provId).attr("data-code","-1");
			jq(_config.cityId).attr("data-code","-1");
			jq(_config.areaId).attr("data-code","-1");
		}
		// change事件 省切换 自动切换市 and 区 
		jq(_config.provId).change(function(e){
			self._cityFunc();
			self._areaFunc();
		});

		// 市 change事件 自动切换区
		jq(_config.cityId).change(function(e){
			self._areaFunc();
		});
		// 区change事件 
		jq(_config.areaId).change(function(){
			jq(_config.areaId).attr("data-code",jq(this).val());
		});
	},
	/*
	 * 渲染省数据
	...
	_provFunc: function(){
		...
		jq(_config.data).each(function(i,curItem){
			elemHtml += "<option data-name='"+curItem[1]+"' value='"+curItem[0]+"'>"+curItem[1]+"</option>";
		});
		jq(_config.provId).html(elemHtml);
		
		if(_config.isSelect) {
			jq(jq(_config.provId + " option")[1]).attr("selected",'true');
			if(this.provCode) {
				self._setSelectCode(_config.provId,this.provCode);
			}
		}
		
		_config.provCallBack && jq.isFunction(_config.provCallBack) && _config.provCallBack();
	},
	/*
	 * 渲染省对应的市数据
	...
	_cityFunc: function(){
		...
		var provIndex = jq(_config.provId).get(0).selectedIndex,   // 获取省份的索引index
			elemHtml = '',
			prov,
			citys,
			provOpt;
		
		provOpt = jq(jq(_config.provId + ' option')[0]).attr("value");
		...
		jq(_config.cityId).empty();
		jq(_config.areaId).empty();
		/*
		...
		if(provIndex < 0){
			...
			jq(_config.cityId).html(opthtml);
			jq(_config.areaId).html(opthtml);
			return;
		}
		...
		jq(_config.cityId).html(elemHtml);

		if(_config.isSelect) {
			jq(jq(_config.cityId + " option")[1]).attr("selected",'true');
			if(this.cityCode) {
		...
		_config.cityCallBack && jq.isFunction(_config.cityCallBack) && _config.cityCallBack();
	},
	/*
	 * 渲染市对应的区数据
	...
	_areaFunc: function(){
		...
		var	provOpt = jq(jq(_config.provId + ' option')[0]).attr("value"),
			cityOpt = jq(jq(_config.cityId + ' option')[0]).attr("value");

		var provIndex = jq(_config.provId).get(0).selectedIndex,
			cityIndex = jq(_config.cityId).get(0).selectedIndex,
			elemHtml = "",
			prov,
			citys,
			areas,
		...
		jq(_config.areaId).html(elemHtml);
		
		if(_config.isSelect) {
			jq(jq(_config.areaId + " option")[1]).attr("selected",'true');
			if(this.areaCode) {
		...
		_config.areaCallBack && jq.isFunction(_config.areaCallBack) && _config.areaCallBack();
	},
	 /*
    ...
    _changeSetCode: function(selectId){
    	...
    	var idx = selectId.selectedIndex, //获取选中的option的索引
            option,
            value,
			dataValue = jq(_config.provId).val();
        ...
        if(idx > -1) {
			if(dataValue == -1) {
				if(provOpt != "-1") {
					jq(_config.provId).prepend(html);
					jq(_config.cityId).prepend(html);
					jq(_config.areaId).prepend(html);
				}
				jq(_config.provId).attr("data-code",'');
				jq(_config.cityId).attr("data-code",'');
				jq(_config.areaId).attr("data-code",'');
			}else {
				option = selectId.options[idx];  //获取选中的option元素
				value = jq(option).val();
				jq(selectId).attr("data-code",value);
			}
        }
    },
    ...
    _setSelectCode: function(selectId,code){
    	...
    	var optionElems = jq(selectId + " option");
		
    	jq(optionElems).each(function(index,item){
    		var curCode = jq(item).val();
    		if(curCode == code) {
    			jq(item).attr("selected",'true');
				jq(selectId).attr("data-code",curCode);
    			return;
    		}
    	});
    }
